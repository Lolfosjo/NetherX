...
final class NetherVegetationPopulator extends Populator
{
    ...
    /**
     * @param int $minClusterSize Minimale Pflanzen pro Cluster (inklusive)
     * @param int $maxClusterSize Maximale Pflanzen pro Cluster (inklusive)
     */
    public function __construct(
        ...
        if ($minClusterSize > $maxClusterSize) {
            throw new \InvalidArgumentException('minClusterSize must not be greater than maxClusterSize.');
        }
    ...
    private function populateClustered(Chunk $chunk, Random $random): void
    {
        ...
        for ($i = 0; $i < $this->attemptsPerChunk; ++$i) {
            ...
            // Größe pro Cluster würfeln.
            $size = $random->nextRange($this->minClusterSize, $this->maxClusterSize);
