...
class Density
{
    ...
    /**
     * @param Random $random Chunk-spezifischer Random, wird für das Bedrock-Muster verwendet.
     */
    public function generateRawTerrain(ChunkManager $world, int $chunkX, int $chunkZ, Random $random): void
    {
    ...
    /**
     * Wie im muqsit/vanillagenerator NetherGenerator: pro Spalte von oben nach unten,
     * zwei getrennte Random-Aufrufe je Y-Ebene (Boden/Decke) mit Short-Circuit.
     * Bedrock überschreibt jeden vorhandenen Block.
     */
    private function placeBedrock(Chunk $chunk, Random $random): void
    {
        $roughness = $this->bedrockRoughness;
        $worldHeightM1 = self::WORLD_HEIGHT - 1;

        for ($x = 0; $x < Chunk::EDGE_LENGTH; ++$x) {
            for ($z = 0; $z < Chunk::EDGE_LENGTH; ++$z) {
                for ($y = $worldHeightM1; $y >= 0; --$y) {
                    if ($y <= $random->nextBoundedInt($roughness)
                        || $y >= $worldHeightM1 - $random->nextBoundedInt($roughness)) {
